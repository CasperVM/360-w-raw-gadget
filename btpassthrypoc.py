...
def find_gamepad():
    ...
    for device in devices:
        if 'gamepad' in device.name.lower():  # Adjust the condition based on your gamepad name
            return device
    return None
...
if gamepad is None:
    raise Exception("Gamepad not found. Ensure it is connected and the name is correctly identified in the script.")
...
try:
    for event in gamepad.read_loop():
        if event.type == ecodes.EV_KEY or event.type == ecodes.EV_ABS:
            report = bytearray(8)  # Example report size, adjust as per your HID descriptor
            if event.type == ecodes.EV_KEY:
                button_map = {304: 0, 305: 1, 307: 2, 308: 3}  # Mapping example, adjust to your setup
